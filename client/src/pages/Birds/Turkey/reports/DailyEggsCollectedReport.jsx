...
const EggsCollectedToday = () => {
  ...
  useEffect(() => {
    fetchEggsCollectedToday();
  }, []);

  const toggleEditModal = () => setEditModal(!editModal);
  const toggleDeleteModal = () => setDeleteModal(!deleteModal);

  const fetchEggsCollectedToday = async () => {
    try {
      const response = await fetch('/api/all-eggs-collected-today');
      if (!response.ok) {
        throw new Error('Failed to fetch eggs collected today');
      }
  ...
  return (
    <Container fluid>
      <h2 className="mt-4">Eggs Collected Today</h2>
...
export default EggsCollectedToday;
