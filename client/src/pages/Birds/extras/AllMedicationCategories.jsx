...
const AllMedicationCategories = () => {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`/api/medication-categories/${id}`, {
        method: 'DELETE',
      });
      if (!response.ok) {
        throw new Error('Failed to delete medication category');
      }
      fetchData();
    } catch (error) {
      console.error(error.message);
  ...
  const handleSave = async () => {
    try {
      const response = await fetch(`/api/medication-categories/${selectedCategory._id}`, {
        method: 'PUT',
        headers: {
      ...
      if (!response.ok) {
        throw new Error('Failed to update medication category');
      }
      toggleModal();
      fetchData();
    } catch (error) {
  ...
  return (
    <Container>
      <h1>All Medication Categories</h1>
      ...
      <Modal isOpen={modal} toggle={toggleModal}>
        <ModalHeader toggle={toggleModal}>Edit Medication Category</ModalHeader>
        <ModalBody>
          <Form>
            <FormGroup>
...
export default AllMedicationCategories;
