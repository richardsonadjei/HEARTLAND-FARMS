...
const AllHatchesWithinPeriod = () => {
  const [hatchedEggs, setHatchedEggs] = useState([]);
  const [errorMessage, setErrorMessage] = useState('');
  const [startDate, setStartDate] = useState('');
  const [endDate, setEndDate] = useState('');

  const fetchHatchedEggsByDateRange = async () => {
    try {
      const response = await fetch(`/api/hatched-eggs-within-range?startDate=${startDate}&endDate=${endDate}`);
      if (!response.ok) {
  ...
  return (
    <Container fluid>
      <h2 className="mt-4">Hatched Eggs Within Period</h2>
      ...
      <div className="mb-4">
        ...
        <input
          ...
          className="form-control"
        />
        <Button color="primary" onClick={fetchHatchedEggsByDateRange} className="mt-2">
          Fetch Records
        </Button>
      </div>
...
export default AllHatchesWithinPeriod;
