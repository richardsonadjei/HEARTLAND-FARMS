...
const CassavaFertilizerApplicationReport = () => {
  ...
  const fetchFertilizerApplications = async () => {
    try {
      const response = await fetch(`/api/view-all-fertilizer-application/${selectedBatch.value}`);
      if (!response.ok) {
        throw new Error('Error fetching cassava fertilizer applications');
      }

      const data = await response.json();
      setFertilizerApplications(data.cassavaFertilizerApplicationRecords);
      setShowReport(true);
    } catch (error) {
      console.error('Error fetching cassava fertilizer applications:', error);
      // Handle error fetching cassava fertilizer applications
    }
  };
    ...
    <Container fluid className="mt-4">
      <Row>
        <Col sm="12" md={{ size: 8, offset: 2 }}>
          <h2 className="mb-4 text-center">Cassava Fertilizer Application Report</h2>
...
export default CassavaFertilizerApplicationReport;
