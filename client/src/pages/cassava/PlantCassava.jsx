...
const PlantCassava = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch('/api/plant-cassava', {
        method: 'POST',
        headers: {
      ...
      // Set success message
      setMessage('Cassava planting and expenditure recorded successfully');
      ...
      console.error('Error recording cassava planting and expenditure:', error);

      // Set error message
      setMessage(`Error recording cassava planting and expenditure: ${error.message}`);
    ...
    <Container>
      <Row>
        <Col sm="12" md={{ size: 8, offset: 2 }}>
          <h2 className="mb-4 text-center">Plant Cassava</h2>
          {message && (
          ...
          <Form onSubmit={handleSubmit}>
            ...
            <Row form>
              ...
              <Col md={6}>
                <FormGroup>
                  <Label for="cassavaVariety">Cassava Variety:</Label>
                  <Input
                    type="text"
                    name="cassavaVariety"
                    id="cassavaVariety"
                    value={formData.cassavaVariety}
                    onChange={handleChange}
                    required
                  />
                </FormGroup>
  ...
  <Select
    name="batchNumber"
    id="batchNumber"
    value={cassavaSeasons.find((season) => season.batchNumber === formData.batchNumber)}
  ...
  />
            ...
            <Button type="submit">Plant Cassava</Button>
          </Form>
        </Col>
      </Row>
    </Container>
