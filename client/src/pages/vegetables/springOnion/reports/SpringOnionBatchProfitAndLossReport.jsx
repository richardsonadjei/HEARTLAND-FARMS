...
const SpringOnionsProfitLossReport = () => {
  const [batch, setBatch] = useState('');
  const [batchOptions, setBatchOptions] = useState([]);
  const [reportData, setReportData] = useState(null);

  useEffect(() => {
    fetchBatchOptions('Spring Onions'); 
  }, []);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`/api/calculate-spring-onion-profit-loss?batchNumber=${batch}`);
      if (!response.ok) {
  ...
  return (
    <Container>
      <h1 className="mt-5 mb-4">Spring Onions Profit Loss Report</h1>
      ...
      {reportData && (
        <div className="mt-4">
          <h2>Profit Loss Report for Spring Onions Of Batch Number: {reportData.sales[0].batchNumber}</h2>
...
export default SpringOnionsProfitLossReport;
