...
const SpringOnionsAllFarmActivitiesReport = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`/api/spring-onions-batch-farm-activities-records/${batchNumber}`);
      if (!response.ok) {
  ...
  const fetchBatchOptions = async () => {
    try {
      const response = await fetch(`/api/all-batches/Spring Onions`);
      if (!response.ok) {
  ...
  return (
    <Container>
      <h2 className="mt-4">Spring Onions All Activities Report</h2>
...
export default SpringOnionsAllFarmActivitiesReport;
