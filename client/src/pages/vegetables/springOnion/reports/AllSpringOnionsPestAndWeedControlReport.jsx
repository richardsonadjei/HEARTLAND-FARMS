...
const AllSpringOnionsPestAndWeedControlReport = () => {
  const [pestAndWeedControlRecords, setPestAndWeedControlRecords] = useState([]);
  const [editModal, setEditModal] = useState(false);
  const [deleteModal, setDeleteModal] = useState(false);
  const [selectedRecord, setSelectedRecord] = useState(null);

  useEffect(() => {
    getAllPestAndWeedControlRecords();
  }, []);

  const toggleEditModal = () => setEditModal(!editModal);
  const toggleDeleteModal = () => setDeleteModal(!deleteModal);

  const getAllPestAndWeedControlRecords = async () => {
    try {
      const response = await fetch('/api/all-spring-onions-pest-and-weed-records');
      if (!response.ok) {
        throw new Error('Failed to fetch pest and weed control records');
      }
      const data = await response.json();
      setPestAndWeedControlRecords(data);
    } catch (error) {
  ...
  const handleSaveChanges = async () => {
    try {
      const response = await fetch(`/api/pest-and-weed-control/${selectedRecord._id}`, {
        method: 'PUT',
        headers: {
      ...
      toggleEditModal();
      // Optionally, you can fetch all pest and weed control records again to refresh the data
      getAllPestAndWeedControlRecords();
    } catch (error) {
      console.error(error.message);
      // Handle error
  ...
  return (
    <Container>
      <h2 className="mt-4">All Pest and Weed Control Records</h2>
      <div className="table-responsive">
        <table className="table">
          ...
          <tbody>
            {pestAndWeedControlRecords.map((record) => (
              <tr key={record._id}>
          ...
          {selectedRecord && (
            <form>
              <div className="form-group">
                <label htmlFor="pestAndWeedControlDetails">Pest and Weed Control Details</label>
                <input type="text" className="form-control" id="pestAndWeedControlDetails" value={selectedRecord.pestAndWeedControlDetails || ''} onChange={(e) => setSelectedRecord({ ...selectedRecord, pestAndWeedControlDetails: e.target.value })} />
              </div>
              <div className="form-group">
...
export default AllSpringOnionsPestAndWeedControlReport;
