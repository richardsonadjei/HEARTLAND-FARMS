...
const SpringOnionsBatchOtherActivitiesRecords = () => {
  const defaultType = 'Spring Onions';
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`/api/spring-onions-other-activities-records/${batchNumber}`);
      if (!response.ok) {
  ...
  return (
    <Container>
      <h2 className="mt-4">Spring Onions Batch Other Activities Records</h2>
...
export default SpringOnionsBatchOtherActivitiesRecords;
