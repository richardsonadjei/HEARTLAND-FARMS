...
const CarrotAllFarmActivitiesReport = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`/api/carrot-batch-farm-activities-records/${batchNumber}`);
      if (!response.ok) {
  ...
  const fetchBatchOptions = async () => {
    try {
      const response = await fetch(`/api/all-batches/Carrot`);
      if (!response.ok) {
  ...
  return (
    <Container>
      <h2 className="mt-4">Carrot All Activities Report</h2>
...
export default CarrotAllFarmActivitiesReport;
