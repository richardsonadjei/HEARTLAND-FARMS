...
const CarrotProfitLossReport = () => {
  const [batch, setBatch] = useState('');
  const [batchOptions, setBatchOptions] = useState([]);
  const [reportData, setReportData] = useState(null);

  useEffect(() => {
    fetchBatchOptions('Carrot'); 
  }, []);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`/api/calculate-carrot-profit-loss?batchNumber=${batch}`);
      if (!response.ok) {
  ...
  return (
    <Container>
      ...
      {reportData && (
        <div className="mt-4">
          <h2>Profit Loss Report for Carrot Of Batch Number: {reportData.sales[0].batchNumber}</h2>
...
export default CarrotProfitLossReport;
