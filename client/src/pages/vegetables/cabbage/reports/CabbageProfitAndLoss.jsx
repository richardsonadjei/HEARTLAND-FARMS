...
const CabbageProfitLossReport = () => {
  const [batch, setBatch] = useState('');
  const [batchOptions, setBatchOptions] = useState([]);
  const [reportData, setReportData] = useState(null);

  useEffect(() => {
    fetchBatchOptions('Cabbage'); // Default to Cabbage
  }, []);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`/api/calculate-cabbage-profit-loss?batchNumber=${batch}`);
      if (!response.ok) {
  ...
  return (
    <Container>
      <h1 className="mt-5 mb-4">Cabbage Profit Loss Report</h1>
      ...
      {reportData && (
        <div className="mt-4">
          <h2>Profit Loss Report for Cabbage Of Batch Number: {reportData.sales[0].batchNumber}</h2>
...
export default CabbageProfitLossReport;
