...
const AllCabbageTransplantingReport = () => {
  ...
  const getAllCabbageTransplantingRecords = async () => {
    try {
      const response = await fetch('/api/all-cabbage-transplanting-records');
      if (!response.ok) {
  ...
  const handleSaveChanges = async () => {
    try {
      const response = await fetch(`/api/cabbage-transplanting/${selectedRecord._id}`, {
        method: 'PUT',
        headers: {
  ...
  return (
    <Container>
      <h2 className="mt-4">All Cabbage Transplanting Records</h2>
...
export default AllCabbageTransplantingReport;
