...
const GreenPepperProfitLossReport = () => {
  const [batch, setBatch] = useState('');
  const [batchOptions, setBatchOptions] = useState([]);
  const [reportData, setReportData] = useState(null);

  useEffect(() => {
    fetchBatchOptions('Green Pepper'); 
  }, []);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`/api/calculate-green-pepper-profit-loss?batchNumber=${batch}`);
      if (!response.ok) {
  ...
  return (
    <Container>
      <h1 className="mt-5 mb-4">Green Pepper Profit Loss Report</h1>
...
export default GreenPepperProfitLossReport;
