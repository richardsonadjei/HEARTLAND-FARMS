...
const GreenPepperAllFarmActivitiesReport = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`/api/green-pepper-batch-farm-activities-records/${batchNumber}`);
      if (!response.ok) {
  ...
  const fetchBatchOptions = async () => {
    try {
      const response = await fetch(`/api/all-batches/Green Pepper`);
      if (!response.ok) {
  ...
  return (
    <Container>
      <h2 className="mt-4">Green Pepper All Activities Report</h2>
...
export default GreenPepperAllFarmActivitiesReport;
