...
const AllBatchActivitiesReport = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`/api/pepper-batch-farm-activities-records/${batchNumber}`);
      if (!response.ok) {
  ...
  const fetchBatchOptions = async () => {
    try {
      const response = await fetch(`/api/all-batches/Pepper`);
      if (!response.ok) {
...
export default AllBatchActivitiesReport;
