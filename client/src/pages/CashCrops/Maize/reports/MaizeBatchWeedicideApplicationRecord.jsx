...
const MaizeBatchWeedicideApplicationRecord = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`/api/maize-batch-weedicide-applications?batchNumber=${batchNumber}&type=${defaultType}`);
      if (!response.ok) {
        throw new Error('Maize weedicide application record not found');
      }
  ...
  return (
    <Container>
      <h2 className="mt-4">Maize Batch Weedicide Application Record</h2>
...
export default MaizeBatchWeedicideApplicationRecord;
