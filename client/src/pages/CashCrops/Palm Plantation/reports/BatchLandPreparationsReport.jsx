...
const BatchLandPreparationRecord = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`/api/cashcrop-batch-land-preparations?batchNumber=${batchNumber}&cashCrop=${defaultType}`);
      if (!response.ok) {
        throw new Error('Land preparation record not found');
      }
  ...
  return (
    <Container>
      <h2 className="mt-4">Batch Land Preparation Record</h2>
...
export default BatchLandPreparationRecord;
