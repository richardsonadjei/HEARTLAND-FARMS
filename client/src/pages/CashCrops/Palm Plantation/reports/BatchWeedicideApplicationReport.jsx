...
const BatchWeedicideApplicationRecord = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`/api/cash-crop-weedicide/${batchNumber}/${defaultType}`);
      if (!response.ok) {
        throw new Error('Weedicide application record not found');
      }
  ...
  const handleEdit = async () => {
    try {
      const response = await fetch(`/api/cash-crop-weedicide-applications/${weedicideRecord._id}`, {
        ...
        body: JSON.stringify(weedicideRecord)
      });
  ...
  return (
    <Container>
      <h2 className="mt-4">Weedicide Application Record</h2>
...
export default BatchWeedicideApplicationRecord;
