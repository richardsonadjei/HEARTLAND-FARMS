...
const BatchPestAndDiseaseControlRecord = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`/api/cashcrop-pest-disease-controls/${batchNumber}/${defaultType}`);
      if (!response.ok) {
        throw new Error('Cash crop pest and disease control record not found');
      }
  ...
  return (
    <Container>
      <h2 className="mt-4">Cash Crop Pest and Disease Control Record</h2>
...
export default BatchPestAndDiseaseControlRecord;
