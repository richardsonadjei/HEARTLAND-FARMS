...
const BatchHarvestRecord = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`/api/cash-crop-harvests/${batchNumber}/${defaultType}`);
      if (!response.ok) {
  ...
  return (
    <Container>
      <h2 className="mt-4">Batch Harvest Record</h2>
          ...
          <table className="table">
            ...
            <tbody>
              <tr>
                ...
                <td>{new Date(harvestRecord.date).toLocaleDateString('en-US', { weekday: 'long', day: 'numeric', month: 'long', year:
'numeric' })}</td>
...
export default BatchHarvestRecord;
