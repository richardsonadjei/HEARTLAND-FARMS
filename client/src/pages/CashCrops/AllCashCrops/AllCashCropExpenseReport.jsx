...
const AllCashCropExpenseReport = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/all-cashcrop-expenses');
      if (!response.ok) {
        throw new Error('Failed to fetch cash crop expenses');
      }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`/api/delete-cashcrop-expense/${id}`, {
        method: 'DELETE',
      });
      if (!response.ok) {
        throw new Error('Failed to delete cash crop expense');
      }
      fetchData();
    } catch (error) {
      console.error(error.message);
  ...
  const handleSave = async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error('Failed to update cash crop expense');
      }
      toggleModal();
      fetchData();
    } catch (error) {
  ...
  return (
    <Container>
      ...
      <Modal isOpen={modal} toggle={toggleModal}>
        <ModalHeader toggle={toggleModal}>Edit Cash Crop Expense</ModalHeader>
        <ModalBody>
          <Form>
            <FormGroup>
...
export default AllCashCropExpenseReport;
