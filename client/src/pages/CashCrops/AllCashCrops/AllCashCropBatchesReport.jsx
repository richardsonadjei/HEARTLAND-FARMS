...
const AllCashCropBatches = () => {
    const [cashCropBatches, setCashCropBatches] = useState({});
    ...
    const fetchData = async () => {
      try {
        const response = await fetch('/api/get-all-batches');
        if (!response.ok) {
          throw new Error('Failed to fetch cash crop batches');
        }
        ...
        setCashCropBatches(batchesByType);
      } catch (error) {
    ...
    const handleDelete = async () => {
      try {
        const response = await fetch(`/api/delete-batch/${selectedBatch._id}`, {
          method: 'DELETE',
        });
        if (!response.ok) {
          throw new Error('Failed to delete cash crop batch');
        }
        toggleDeleteModal();
        fetchData();
      } catch (error) {
    ...
    const handleSaveEdit = async () => {
      try {
        const response = await fetch(`/api/update-batch/${selectedBatch._id}`, {
          method: 'PUT',
          headers: {
        ...
        if (!response.ok) {
          throw new Error('Failed to update cash crop batch');
        }
        toggleEditModal();
        fetchData();
      } catch (error) {
  ...
  const formatDate = (dateString) => {
    ...
    return (
      <Container>
        <h1 style={{ textAlign: 'center', color: 'blue' }}>All Cash Crop Batches</h1>

        {Object.keys(cashCropBatches).map((type) => (
          <div key={type}>
            ...
            <Table responsive striped hover>
              ...
              <tbody>
                {cashCropBatches[type].map((batch, index) => (
                  <tr key={batch._id}>
  ...
  export default AllCashCropBatches;
