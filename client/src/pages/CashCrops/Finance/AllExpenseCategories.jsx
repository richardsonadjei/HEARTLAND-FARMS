...
const AllExpenseCategories = () => {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`/api/delete-expense-categories/${id}`, {
        method: 'DELETE',
      });
      if (!response.ok) {
  ...
  const handleSave = async () => {
    try {
      const response = await fetch(`/api/edit-expense-categories/${selectedCategory._id}`, {
        method: 'PUT',
        headers: {
  ...
  // Sort expenseCategories by name in alphabetical order
  const sortedExpenseCategories = [...expenseCategories].sort((a, b) => a.name.localeCompare(b.name));
