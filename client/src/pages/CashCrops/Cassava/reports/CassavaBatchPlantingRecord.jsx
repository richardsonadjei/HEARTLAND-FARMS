...
const CassavaBatchPlantingRecord = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`/api/cassava-batch-planting?batchNumber=${batchNumber}&type=${defaultType}`);
      if (!response.ok) {
        throw new Error('Maize planting record not found');
      }
  ...
  return (
    <Container>
      <h2 className="mt-4">Maize Batch Planting Record</h2>
...
export default CassavaBatchPlantingRecord;
