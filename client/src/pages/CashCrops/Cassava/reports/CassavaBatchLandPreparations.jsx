...
const CassavaBatchLandPreparationRecord= () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`/api/cassava-batch-land-preparations?batchNumber=${batchNumber}&cashCrop=${defaultType}`);
      if (!response.ok) {
  ...
  return (
    <Container>
      <h2 className="mt-4">Cassava Batch Land Preparation Record</h2>
...
export default CassavaBatchLandPreparationRecord;
