...
const CassavaBatchHarvestRecord = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`/api/cassava-batch-harvest-records?batchNumber=${batchNumber}&type=${defaultType}`);
      if (!response.ok) {
  ...
  return (
    <Container>
      <h2 className="mt-4">Cassava Batch Harvest Record</h2>
...
export default CassavaBatchHarvestRecord;
