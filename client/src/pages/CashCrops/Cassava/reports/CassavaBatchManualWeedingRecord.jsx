...
const CassavaBatchManualWeedingReport = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`/api/cassava-batch-manual-weeding?batchNumber=${batchNumber}&type=${defaultType}`);
      if (!response.ok) {
        throw new Error('Maize manual weeding record not found');
      }
  ...
  return (
    <Container>
      <h2 className="mt-4">Cassava Batch Manual Weeding Report</h2>
...
export default CassavaBatchManualWeedingReport;
