...
const PlantainBatchFertilizerApplicationRecord = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`/api/plantain-batch-fertilizer-application-report?batchNumber=${batchNumber}&type=${defaultType}`);
      if (!response.ok) {
        throw new Error('Maize fertilizer application record not found');
      }
  ...
  const handleEdit = async () => {
    try {
      const response = await fetch(`/api/cash-crop-fertilizer-applications/${fertilizerRecord._id}`, {
        method: 'PUT',
        headers: {
  ...
  return (
    <Container>
      <h2 className="mt-4">Plantain Batch Fertilizer Application Record</h2>
...
export default PlantainBatchFertilizerApplicationRecord;
