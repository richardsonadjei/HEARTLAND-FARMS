...
const PlantainBatchPlantingRecord = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`/api/plantain-batch-planting?batchNumber=${batchNumber}&type=${defaultType}`);
      if (!response.ok) {
        throw new Error('Plantain planting record not found');
      }
  ...
  return (
    <Container>
      <h2 className="mt-4">Plantain Batch Planting Record</h2>
...
export default PlantainBatchPlantingRecord;
