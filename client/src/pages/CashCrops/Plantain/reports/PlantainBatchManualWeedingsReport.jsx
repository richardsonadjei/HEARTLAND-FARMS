...
const PlantainBatchManualWeedingReport = () => {
  const defaultType = 'Plantain';
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`/api/plantain-batch-manual-weeding?batchNumber=${batchNumber}&type=${defaultType}`);
      if (!response.ok) {
        throw new Error('Plantain manual weeding record not found');
      }
  ...
  const handleEdit = async () => {
    try {
      const response = await fetch(`/api/cash-crop-manual-weeding/${weedingRecord._id}`, {
        method: 'PUT',
        headers: {
  ...
  return (
    <Container>
      <h2 className="mt-4">Plantain Batch Manual Weeding Report</h2>
            ...
            <ModalBody>
              <Form>
                ...
                <FormGroup>
                  ...
                  <Input type="text" name="recordedBy" id="recordedBy" value={weedingRecord.recordedBy || ''} onChange={(e) => setWendingRecord({ ...weedingRecord, recordedBy: e.target.value })} />
</FormGroup>
</Form>
</ModalBody>
<ModalFooter>
...
export default PlantainBatchManualWeedingReport;
