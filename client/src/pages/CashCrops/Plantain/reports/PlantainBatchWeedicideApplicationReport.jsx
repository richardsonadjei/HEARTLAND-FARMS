...
const PlantainBatchWeedicideApplicationRecord = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`/api/plantain-batch-weedicide-applications?batchNumber=${batchNumber}&type=${defaultType}`);
      if (!response.ok) {
        throw new Error('Plantain weedicide application record not found');
      }
  ...
  return (
    <Container>
      <h2 className="mt-4">Plantain Batch Weedicide Application Record</h2>
...
export default PlantainBatchWeedicideApplicationRecord;
