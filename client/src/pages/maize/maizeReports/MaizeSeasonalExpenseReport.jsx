...
const SeasonalMaizeExpenseReport = () => {
  ...
  const fetchExpenses = async () => {
    try {
      const response = await fetch(`/api/maize-seasonal-expense-report/${selectedBatch.value}`);
      if (!response.ok) {
        throw new Error('Error fetching maize farm expenses');
      }
      ...
      // Handle error fetching maize farm expenses
    }
  };
    ...
    <Container fluid className="mt-4">
      <Row>
        <Col sm="12" md={{ size: 8, offset: 2 }}>
          <h2 className="mb-4 text-center">Seasonal Maize Expense Report</h2>
...
export default SeasonalMaizeExpenseReport;
