...
const SeasonalMaizeProfitLossReport = () => {
  ...
  const fetchBatchNumbers = async () => {
    try {
      const response = await fetch('/api/all-maize-seasons'); // Replace with the actual endpoint
      if (!response.ok) {
  ...
  const handleSubmit = async () => {
    try {
      const response = await fetch(`/api/calculate-maize-ProfitLoss?batchNumber=${batchNumber}`);
      const data = await response.json();
      setReportData(data);
    } catch (error) {
  ...
  const renderTable = () => {
    return (
      <div>
        <h3>Seasonal Maize Profit/Loss Report for Batch Number {batchNumber}</h3>
...
export default SeasonalMaizeProfitLossReport;
